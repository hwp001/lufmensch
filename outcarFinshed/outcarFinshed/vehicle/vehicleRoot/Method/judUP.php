 ...
 */
    ...
    */
    ...
    //验证车牌号
    public static function isCarLicense($value){
        $value = trim($value);
        $match = '/^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}[A-Z0-9]{4}[A-Z0-9挂学警港澳]{1}$/u';

       return preg_match($match, $value);
    }
    //除去前面的0
    public static function isNoZero($value){
        $str = $value;
        for ($i=0; $i < strlen($value); $i++) { 
            if ($value[$i] == 0) {
