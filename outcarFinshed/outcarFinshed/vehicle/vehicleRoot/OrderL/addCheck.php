...
$colum['existState'] = $_POST['existState'];
$str1 = $_POST['beginTime1'][0];
$str2 = $_POST['beginTime1'][1];
//判断时分格式范围s
if (!($str1 > 0 && $str1 < 24) || !($str2 > 0 && $str2 < 60)) {
	echo "<script>alert('时分格式出错，请重新填写');history.back();</script>";die;
}
//除去前缀零
$verify = new Verify();
...
if ($colum['createTime'] > $colum['beginTime']) {
	echo "<script>alert('出发时间不能在订单生成时间之前');window.location.href='./index.php'</script>";
}
...
while (($row = $res->fetch_assoc()) && mysqli_affected_rows($addNode->mysqli)>0) {
	 ...
	 // var_dump($Yes);die;
	if ($Yes<0) {
		echo "<script>alert('订单货量超出，请重新输入，谢谢');history.back();</script>";die;
	} else {
		if ($c == 1) {//增加订单
			$colum['createTime'] = time();	
			...
			$yes = $addNode->_updateOrder($colum,'vehicle_order');
			if($yes){
			 $good['goodId'] = $_POST['goodId'];
			 $good['goodName'] = $row['goodName'];
			 $good['goodNum'] = $row['goodNum'] - $colum['goodCount'];
